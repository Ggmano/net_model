...
def box_ciou(b1, b2):
    ...
    # torch.zeros_like(intersect_maxes)：防止出现负值
    intersect_area = intersect_wh[..., 0] * intersect_wh[..., 1]

    b1_area = b1_wh[..., 0] * b1_wh[..., 1]
    b2_area = b2_wh[..., 0] * b2_wh[..., 1]
    union_area = b1_area + b2_area - intersect_area
    iou = intersect_area / torch.clamp(union_area, min=1e-6)  # 防止出现分母为零的情况
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        #----------------------------------------------------#
        #            size   (0)       (1)         (2) (3)
        #   input的shape为  bs, 3*(5+num_classes), 13, 13       每个yolo head的loss是单独计算的
        #                   bs, 3*(5+num_classes), 26, 26
        #                   bs, 3*(5+num_classes), 52, 52
        #
        #  targets的shape为 num_obj , 4---[x,y,w,h,c]
        #----------------------------------------------------#
        ...
        #   如果特征层为13x13的话，一个特征点就对应原来的图片上的32x32个像素点
        #   如果特征层为26x26的话，一个特征点就对应原来的图片上的16x16个像素点
        #   如果特征层为52x52的话，一个特征点就对应原来的图片上的8x8个像素点
        #   stride_h = stride_w = 32 or 16 or 8
        #-----------------------------------------------------------------------#
        stride_h = self.img_size[1] / in_h
        stride_w = self.img_size[0] / in_w

        #-------------------------------------------------#
        #   此时获得的scaled_anchors大小是 相对于 特征层的   缩小anchors
        #-------------------------------------------------#
        ...
        #   batch_size, 3 * 5 + num_classes, 13, 13  (input.shape)
        #   batch_size, 3, 26, 26, 5 + num_classes
        #   batch_size, 3, 52, 52, 5 + num_classes
        #-----------------------------------------------#
        # [batch_size,3*5+numclass,13,13]--->[bs,3,13,13,5+numclass]
        #  view 方法要求Tensor是连续的-----contiguous()
        ...
        # self.get_target()----> def get_target(),不用self的话找不到定义，同属于Class下的函数
        ...
        #   作为负样本不合适
        #   yolov3的想法：一个anchor对应一个GT,这样会导致正负样本不均衡的问题
        #   正样本：与GT的IOU最大的框。
        #   负样本：与GT的IOU<0.5 的框。
        ...
        loss_cls = torch.sum(BCELoss(pred_cls[mask == 1], smooth_labels(
            tcls[mask == 1], self.label_smooth, self.num_classes)))

        loss = loss_conf * self.lambda_conf + loss_cls * \
            self.lambda_cls + loss_loc * self.lambda_loc
    ...
    # woc ,这里该不会是真正构造targets的位置把,居然藏在loss里面
    ...
    def get_target(self, target, anchors, in_w, in_h, ignore_threshold):
        ...
        #-------------------------------------------------------#
        # 如果特征图的尺寸是13，就选[0,1,2]
        # feature_length = [13,26,52]
        # self.feature_length.index(in_w)--(返回值：索引值)  feature_length[索引值]=in_w
        anchor_index = [[0, 1, 2], [3, 4, 5], [
            6, 7, 8]][self.feature_length.index(in_w)]
        subtract_index = [0, 3, 6][self.feature_length.index(in_w)]  # 0
        #-------------------------------------------------------#
        #   创建全是0或者全是1的阵列
        #-------------------------------------------------------#
        mask = torch.zeros(bs, int(self.num_anchors/3),
                           in_h, in_w, requires_grad=False)  # [batch_size,3,13,13]
        ...
        t_box = torch.zeros(bs, int(self.num_anchors/3),
                            in_h, in_w, 4, requires_grad=False)  # [batch_size,3,13,13,4]

        tconf = torch.zeros(bs, int(self.num_anchors/3),
                            in_h, in_w, requires_grad=False)  # [batch_size,3,13,13]
        tcls = torch.zeros(bs, int(self.num_anchors/3), in_h,
                           in_w, self.num_classes, requires_grad=False)  # [batch_size,3,13,13,num_classes]
        ...
        for b in range(bs):
            ...
            #   计算出正样本属于特征层的哪个特征点
            #-------------------------------------------------------#
            # torch.floor()向下取整，看中心点在哪个cell中
            gis = torch.floor(gxs)
            gjs = torch.floor(gys)

            #-------------------------------------------------------#
            #   将真实框转换一个形式---将左上角，移动到原点，x=0,y=0
            #   num_true_box, 4
            #-------------------------------------------------------#
            # gt_box = [num_obg,4]
            ...
            #   num_true_box,
            #-------------------------------------------------------#
            best_ns = torch.argmax(anch_ious, dim=-1)  # 最大iou对应的索引
            for i, best_n in enumerate(best_ns):
                if best_n not in anchor_index:  # 13*13 特征层对应[]
                    continue
                #-------------------------------------------------------------#
                #   取出各类坐标：
                ...
                if (gj < in_h) and (gi < in_w):
                    # 如果是26 or 52 的情况下，将其限制在（0，3），我觉得可以直接用取余来做，best_n = best_n % 3
                    best_n = best_n - subtract_index
                    #----------------------------------------#
                    #   noobj_mask代表无目标的特征点 (一开始全是1，全是没有目标，把没有目标的点扣去，相当于这个点有目标)
                    #----------------------------------------#
                    ...
                    box_loss_scale_y[b, best_n, gj,
                                     gi] = target[b][i, 3]  # h  归一化
                    #----------------------------------------#
                    #   tconf代表物体置信度
                    #----------------------------------------#
        ...
        #   tconf       batch_size, 3, in_h, in_w   置信度真实值
        #   tcls        batch_size, 3, in_h, in_w, num_classes  种类真实值
        # box_loss_scale_x  batch_size, 3, in_h, in_w
        # box_loss_scale_x  batch_size, 3, in_h, in_w
        #----------------------------------------------------------------#
