...
class CSPDarkNet(nn.Module):
    def __init__(self, layers):
        ...
        self.stages = nn.ModuleList([
            ...
            Resblock_body(
                self.feature_channels[0], self.feature_channels[1], layers[1], first=False),
            # 104,104,128 -> 52,52,256
            Resblock_body(
                self.feature_channels[1], self.feature_channels[2], layers[2], first=False),
            # 52,52,256 -> 26,26,512
            Resblock_body(
                self.feature_channels[2], self.feature_channels[3], layers[3], first=False),
            # 26,26,512 -> 13,13,1024
            Resblock_body(
                self.feature_channels[3], self.feature_channels[4], layers[4], first=False)
        ])
    ...
    def forward(self, x):
        ...
        x = self.stages[0](x)
        x = self.stages[1](x)
        out3 = self.stages[2](x)   # 52x52x256
        out4 = self.stages[3](out3)  # 26x26x512
        out5 = self.stages[4](out4)  # 13x13X1024
